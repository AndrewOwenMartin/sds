...
def synchronous_iterate(
    ...
    random_hypothesis_function,
    diffusion_phase,
    test_phase=standard_test_phase,
    multidiffusion=1,
    multitesting=1,
    multitest_function=None,
    rng=random,
...
iteration."""
    ...
    while True:
        ...
        diffusion_phase_iterator = diffusion_phase(
            swarm=swarm,
        ...
        test_phase_iterator = test_phase(
            swarm=swarm,
            microtests=microtests,
            multitesting=multitesting,
            multitest_function=multitest_function,
...
runs for a certain amount of wall clock time."""
    ...
    context_free = diffusion_phase is context_free_diffusion

    context_sensitive = (
        diffusion_phase is context_sensitive_diffusion
    )

    compare = test_phase is comparative_test_phase
...
def run(
    ...
    random_hypothesis_function,
    max_iterations=1000,
    diffusion_phase=passive_diffusion,
...
:param microtests: A collection of functions all of which take a
hypothesis, as returned by random_hypothesis_function and return the
result of a microtest as either a scalar value or a boolean. All tests
must return the same type.
:param random_hypothesis_function: A function which takes a random
number generator and returns hypothesis suitable as input for all the
functions in microtests.
:param max_iterations: The number of iterations afterwhich the SDS will
halt.  If max_iterations=None, the SDS will never halt due to the number
of iterations, but may be manually halted, or halted by the halting
function.
:param diffusion_phase: The diffusion function to use in the diffusion
phase.
:param random: A random number generator, probably an instance of the
random.Random class. Use an instance with an explicit seed to get
repeatable behaviour, else you may pass in the random module itself.
...
value (e.g. None, 0, False, [], '') then the halting_function is never
called.
...
list of microtest results and turns them into a single result. The most
likely values for multitest_function will be 'all' i.e. all microtests
must pass, and 'any' i.e. at least one microtest must pass.
...
clusters.
:param test_phase: (Default: standard_test_phase) The function to use in
the test phase.
:param hypothesis_string_function: (Default: str) The function to call
on a hypothesis to turn it into a string, suitable for inclusion in the
report. If hypotheses are using built-in data types, str is often
enough, otherwise a custom 'to string' function must be supplied.
...
tested with the newest version of sds.run."""
    ...
    for swarm, diffusion_phase, random_hypothesis_function in zip(
        swarms,
        diffusion_phases,
        random_hypothesis_functions
    ):
        diffusion_phase(swarm, random, random_hypothesis_function)
